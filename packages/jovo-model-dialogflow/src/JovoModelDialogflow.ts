...
export class JovoModelDialogflow extends JovoModel {
  ...
  static toJovoModel(
      inputData: NativeFileInformation[],
      locale: string
  ): JovoModelDialogflowData {
      ...
      for (fileInformation of intentFiles) {
          ...
          JovoModelDialogflow.skipDefaultIntentProps(
              jovoIntent,
              dialogFlowIntent,
              locale
          );
          ...
          const userSaysFile = intentFiles.find(file => {
              if (
                  file.path[1] ===
                  dialogFlowIntent.name + "_usersays_" + locale + ".json"
              ) {
                  return true;
              }
      ...
      for (fileInformation of entityFiles) {
          ...
          // skip default intent properties
          JovoModelDialogflow.skipDefaultEntityProps(
              jovoInput,
              dialogFlowEntity
          );
          ...
          const userSaysFile = entityFiles.find(file => {
              if (
                  file.path[1] ===
                  dialogFlowEntity.name + "_entries_" + locale + ".json"
              ) {
                  return true;
              }
      ...
      for (const intent of (model.intents || []) as IntentDialogflow[]) {
              ...
              for (const input of intent.inputs) {
                  ...
                  if (typeof input.type === "object") {
                      if (input.type.dialogflow) {
                          ...
                          throw new Error(
                              'Please add a dialogflow property for input "' +
                                  input.name +
                                  '"'
                          );
                      }
                  ...
                  if (parameterObj.dataType === "") {
                      ...
                      const matchedInputTypes = model.inputTypes.filter(
                          (item: InputType) => {
                      ...
                      for (const matchedInputType of matchedInputTypes) {
                          ...
                          if (matchedInputType.dialogflow) {
                              if (
                                  typeof matchedInputType.dialogflow ===
                                  "string"
                              ) {
                                  dfEntityObj.name =
                          ...
                          if (
                              matchedInputType.values &&
                              matchedInputType.values.length > 0
                          ) {
                              const entityValues = [];
          ...
          if (dialogFlowIntentUserSays.length > 0) {
              returnFiles.push({
                  path: [
                      "intents",
                      intent.name + "_usersays_" + locale + ".json"
                  ],
