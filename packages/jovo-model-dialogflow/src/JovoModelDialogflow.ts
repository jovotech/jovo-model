...
export class JovoModelDialogflow extends JovoModel {
      ...
      if (intent.inputs) {
        ...
        for (const input of intent.inputs) {
          ...
          if (parameterObj.dataType === '') {
            ...
            if (!model.inputTypes) {
              throw new Error(
                'Input type "' + parameterObj.dataType + '" must be defined in inputTypes',
              );
            }
  ...
  static skipDefaultIntentProps(
    ...
    if (
      _.difference(_.get(dialogFlowIntent, 'contexts'), _.get(DEFAULT_INTENT, 'contexts')).length >
      0
    ) {
    ...
    if (
      ...
      webhookForSlotFilling !== _.get(DEFAULT_INTENT, 'webhookForSlotFilling')
    ) {
    ...
    if (
      _.difference(_.get(dialogFlowIntent, 'events'), _.get(DEFAULT_INTENT, 'events')).length > 0
    ) {
    ...
    if (
      ...
      if (
        ...
        !_.isEqual(resetContexts, _.get(DEFAULT_INTENT, 'responses[0].resetContexts'))
      ) {
      ...
      if (
        ...
        !_.isEqual(affectedContexts, _.get(DEFAULT_INTENT, 'responses[0].affectedContexts'))
      ) {
      ...
      if (
        ...
        !_.isEqual(
          defaultResponsePlatforms,
          _.get(DEFAULT_INTENT, 'responses[0].defaultResponsePlatforms'),
        )
      ) {
        _.set(
          jovoIntent,
      ...
      if (
        ...
        !_.isEqual(responseSpeech, _.get(DEFAULT_INTENT, 'responses[0].speech'))
      ) {
  ...
  static skipDefaultEntityProps(jovoInput: InputType, dialogflowEntity: DialogflowLMEntity) {
    ...
    if (
      _.get(dialogflowEntity, 'automatedExpansion') !== _.get(DEFAULT_ENTITY, 'automatedExpansion')
    ) {
      _.set(
        jovoInput,
